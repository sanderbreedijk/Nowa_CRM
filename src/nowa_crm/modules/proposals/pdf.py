...
def _background(canvas,doc,profile,proposal,cover=False):
    ...
    if doc.page==1:
        canvas.setFillColor(BLUE);canvas.setFont("Helvetica-Bold",14);canvas.drawString(17*mm,h-18*mm,(profile.get("company_name") or "NOWA Solutions").lower())
        ...
        canvas.setFillColor(colors.HexColor("#EAF5FB"));canvas.circle(w-18*mm,76*mm,44*mm,fill=1,stroke=0)
        canvas.setFillColor(colors.HexColor("#F4F8FA"));canvas.circle(w-35*mm,46*mm,27*mm,fill=1,stroke=0)
        canvas.setFillColor(BLUE);canvas.rect(0,0,w,28*mm,fill=1,stroke=0)
        canvas.setFillColor(colors.white);canvas.setFont("Helvetica",8);canvas.drawString(17*mm,14*mm,profile.get("footer_text") or profile.get("company_name") or "NOWA Solutions")
    else:
