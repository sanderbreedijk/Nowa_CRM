...
def export_proposal_pdf(service:ProposalService,proposal_id:int,output_dir:Path|None=None)->Path:
    ...
    all_lines=[x for x in service.lines(proposal_id) if x.active];options=[x for x in all_lines if x.optional];lines=[x for x in all_lines if not x.optional];once=[x for x in lines if x.billing_period=="eenmalig"];monthly=[x for x in lines if x.billing_period=="maandelijks"]
    ...
    if options:
        story.append(_p("Optionele uitbreidingen",s["H2N"]));option_rows=[["Omschrijving","Aantal","Periode","Bedrag excl. BTW"]]+[[x.description,f"{x.quantity:g}",x.billing_period,_money(x.line_total_cents)] for x in options];story.append(_table(option_rows,[82*mm,20*mm,30*mm,38*mm],s,True,(1,3)));story.append(_p("Optionele regels zijn niet in het offertetotaal meegerekend en worden alleen na afzonderlijk akkoord uitgevoerd.",s["SmallN"]))
