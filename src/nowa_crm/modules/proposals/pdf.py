...
def _money(cents:int)->str:
    return f"â‚¬ {cents/100:,.2f}".replace(",","X").replace(".",",").replace("X",".")
...
def _bullets(story,items,s):
    for item in items:story.append(_p("â€¢ "+item,s["BodyN"]))
...
def export_proposal_pdf(service:ProposalService,proposal_id:int,output_dir:Path|None=None)->Path:
    ...
    hourrows.append(["Totaal arbeid","",f"{labour_hours:g}","",_money(labour_cents)]);story.append(_table(hourrows,[65*mm,37*mm,17*mm,25*mm,28*mm],s,True,(2,3,4)));story.append(_p("Deze uren dekken voorbereiding, coÃ¶rdinatie, communicatie, uitvoering, documentatie, oplevering en kwaliteitscontrole.",s["BodyN"]))
