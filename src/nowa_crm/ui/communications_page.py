...
class CommunicationsPage(QWidget):
    def __init__(self, customers: CustomerService, service: CommunicationService, open_mail, open_call, parent=None):
        ...
        actions.addWidget(open_item); actions.addWidget(mail); actions.addWidget(call); actions.addStretch()
    ...
    def refresh(self, *_):
        ...
        for row_index, row in enumerate(rows):
            values = (row["date"], row["channel"], row["customer"], row["direction"], row["subject"],
                      row["contact"], row["status"], row["id"], row["channel"])
