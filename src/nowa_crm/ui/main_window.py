...
class MainWindow(QMainWindow):
    ...
    def _build_navigation(self,nav,pages):
        ...
        for section,indices in groups:
            header=QPushButton(f"â€º  {section}");header.setObjectName("NavSection");nav.addWidget(header)
    ...
    def _open_nav_section(self,name):
        for section,(header,content) in self.nav_sections.items():
            active=section==name;content.setVisible(active);header.setText(f"{'âŒ„' if active else 'â€º'}  {section}")
