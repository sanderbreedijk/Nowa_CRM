...
class MainWindow(QMainWindow):
    def __init__(self, customers: CustomerService, proposals: ProposalService, vault: VaultService, operations: OperationsService, workspace: WorkspaceService, mail: MailService, telephony: TelephonyService):
        ...
        call_row=QHBoxLayout();self.call_duration=QLabel("00:00:00");self.call_duration.setObjectName("ActiveCallDuration");end_call=QPushButton("Gesprek beÃ«indigen");end_call.setObjectName("EndCall");end_call.clicked.connect(self.end_active_call)
        ...
        self.sip_nav_status=QLabel("SIP  â€¢  niet actief");self.sip_nav_status.setObjectName("SipSidebarStatus");self.sip_nav_status.setProperty("sipState","idle");nav.addWidget(self.sip_nav_status)
        self.integrations_page.sip_status_changed.connect(self.update_sip_sidebar_status)
        version=QLabel(f"Versie {__version__}  â€¢  lokaal"); version.setObjectName("SidebarFooter"); nav.addWidget(version); shell.addWidget(self.sidebar); shell.addWidget(self.stack,1); self.setCentralWidget(root);self._polish_ui()
    ...
    def update_sip_sidebar_status(self,state,detail=""):
        ...
        self.sip_nav_status.setText(f"SIP  â€¢  {labels.get(state,state)}")
    ...
    def _update_page(self):
        ...
        self.update_status=QLabel("De broncode en releases staan privÃ© op GitHub. Meld je daar aan om een Windows-updatepakket te downloaden."); self.update_status.setWordWrap(True); content.addWidget(self.update_status)
    ...
    def _dashboard(self):
        page,box=self._page("Dagstart","Alles wat vandaag aandacht nodig heeft, lokaal in Ã©Ã©n werkbak."); grid=QGridLayout(); grid.setHorizontalSpacing(14); grid.setVerticalSpacing(14); self.kpis=[]
        ...
        for i,(symbol,title,accent,page_index) in enumerate(card_data):
            ...
            bottom=QHBoxLayout();label=QLabel(title);label.setObjectName("KpiLabel");hint=QLabel("Open  â†’");hint.setObjectName("CardLink")
        ...
        self.day_owner=QLineEdit();self.day_owner.setPlaceholderText("Zoek medewerkerâ€¦");self.day_owner.textChanged.connect(self.refresh_daystart)
        ...
        empty=QFrame(); empty.setObjectName("EmptyState"); empty_box=QVBoxLayout(empty); empty_box.setAlignment(Qt.AlignCenter); e…8070 tokens truncated…  self.active_call_id=call_id;self.call_name.setText(f"{call['customer_name']}\n{call['phone_number']}")
    ...
    def handle_call_finished(self,call_id,call):
        ...
        self.mail_page.to.setText(recipient);self.mail_page.subject.setText(f"Samenvatting telefoongesprek â€“ {subject}");self.mail_page.body.setPlainText(body);self._show(7)
    ...
    def reveal_secret(self,*_):
        ...
        try:
            ...
            secret=self.vault.reveal(entry_id,verification.request_reason,verification_id);alphabet="  ".join(f"{ch} â€” {self._spell(ch)}" for ch in secret);copied=verification.delivery_mode=="copy"
    ...
    def install_local_update(self):
        ...
        try:
            self.update_status.setText("Het lokale updatepakket wordt veilig gecontroleerdâ€¦");QApplication.processEvents()
    ...
    def refresh_backup_status(self):
        ...
        self.backup_status.setText(f"Laatste herstelset: {latest.created_at} Â· {latest.files} bestanden Â· {size:.1f} MB Â· {status}.")
    def create_recovery_backup(self):
        try:
            self.backup_status.setText("Database, kluis en klantbestanden worden lokaal veiliggesteldâ€¦");QApplication.processEvents()
    ...
    def restore_recovery_backup(self):
        ...
        try:
            ...
            answer=QMessageBox.warning(
                ...
                f"Herstelset van {info.created_at}\n{info.files} bestanden Â· {size:.1f} MB\n\n"
    ...
    def refresh_all(self):
        ...
        if hasattr(self,"missed_calls_page"):
            ...
            self.nav_buttons[19].setText(f"Gemiste oproepen  Â·  {missed}" if missed else "Gemiste oproepen")
        self.refresh_backup_status()
