...
class MainWindow(QMainWindow):
    ...
    def _open_nav_section(self,name):
        for section,(header,content) in self.nav_sections.items():
            active=section==name;content.setVisible(active);header.setText(f"{'â€”' if active else '+'}  {section.upper()}")
    ...
    def _dashboard(self):
        page,box=self._page("Dagstart","Alles wat vandaag aandacht nodig heeft, lokaal in Ã©Ã©n werkbak."); grid=QGridLayout(); self.kpis=[]
        ...
        box.addLayout(grid)
        filters=QHBoxLayout();self.day_owner=QLineEdit();self.day_owner.setPlaceholderText("Filter op medewerkerâ€¦");self.day_owner.textChanged.connect(self.refresh_daystart)
    ...
    def refresh_daystart(self,*_):
        ...
        summary=self.daystart_service.summary();self.day_summary.setText(f"{summary['total']} open Â· {summary['overdue']} te laat Â· {summary['urgent']} urgent")
    ...
    def _vault_page(self):
        ...
        self.vault_table=QTableWidget(0,9); self.vault_table.setHorizontalHeaderLabels(["Klant","Klantnr.","Categorie","Groep","Omschrijving","Gebruikersnaam","Host / IP","URL","ID"]); self.vault_table.setColumnHidden(8,True); se…639 tokens truncated…enFileName(self,"GeÃ«xtraheerde oude offerte selecteren","","NOWA-offertepakket (*.zip)")
        ...
        try:
            ...
            labels=[f"{customer.customer_number} â€” {customer.name}" for customer in customers]
            label,ok=QInputDialog.getItem(self,"Klant voor oude offerte",f"Kies de klant voor {preview.source_number} â€” {preview.title}",labels,0,False)
            if not ok:return
            customer=customers[labels.index(label)]
            line_summary="\n".join(
                f"â€¢ {line['description']}: {line['quantity']:g} Ã— â‚¬ {line['unit_price_cents']/100:,.2f}"
                for line in preview.lines[:12]
            )
            message=(
                f"GEKOZEN KLANT\n{customer.customer_number} â€” {customer.name}\n\n"
                f"OUDE OFFERTE\nNummer: {preview.source_number}\nDatum: {preview.source_date or 'onbekend'}\n"
                f"Titel: {preview.title}\nUren: {preview.labor_hours:g}\n"
                f"Totaal excl. btw: â‚¬ {preview.subtotal_cents/100:,.2f}\n\n"
            ...
            warning_text=("\n\nAandachtspunten:\n"+"\n".join(f"â€¢ {x}" for x in result["warnings"])) if result["warnings"] else ""
            QMessageBox.information(
                self,"Oude offerte geÃ¯mporteerd",
    ...
    def import_customers(self):
        ...
        try:
            ...
            detail="\n".join(f"â€¢ {item['action']}: {item['customer_number']} â€” {item['name']} ({item['fields']})" for item in preview.changes[:25])
            if len(preview.changes)>25:detail+=f"\nâ€¢ â€¦ en nog {len(preview.changes)-25} wijzigingen"
            warnings=("\n\nWAARSCHUWINGEN\n"+"\n".join(f"â€¢ {text}" for text in preview.warnings)) if preview.warnings else ""
            message=(f"Bronregels: {len(preview.rows)}\n\n"
                     ...
                     "VÃ³Ã³r uitvoering wordt automatisch een lokale back-up gemaakt.\n\nSynchronisatie uitvoeren?")
    ...
    def customer_import_history(self):
        ...
        labels=[f"#{run['id']} â€” {run['created_at']} â€” {run['source_name']} â€” {run['status']}" for run in runs]
        ...
        lines=[f"{item['action'].capitalize()}: {item['customer_number']} â€” {item['customer_name']}"
    ...
    def export_customers(self):
        ...
        try:
            ...
            QMessageBox.information(self,"Klanten geÃ«xporteerd",f"De actuele actieve klantenlijst is opgeslagen:\n{path}")
    ...
    def open_global_result(self,kind,entity_id,customer_id,title):
        ...
        elif kind=="Kluis":self.open_vault(title.split(" Â· ",1)[0])
        elif kind=="Document":
    ...
    def reveal_secret(self,*_):
        ...
        requester,ok=QInputDialog.getText(self,"Stap 1 van 5 â€” Aanvrager","Naam van de persoon die het wachtwoord aanvraagt")
        if not ok:return
        methods=list(self.vault.VERIFICATION_METHODS);method,ok=QInputDialog.getItem(self,"Stap 2 van 5 â€” Verificatie","Gebruikte verificatiemethode",methods,0,False)
        if not ok:return
        reason,ok=QInputDialog.getText(self,"Stap 3 van 5 â€” Reden","Waarom heeft de beller dit wachtwoord nodig?")
        if not ok:return
        identity=QMessageBox.question(self,"Stap 4 van 5 â€” Identiteit","Is de identiteit daadwerkelijk gecontroleerd met de gekozen methode?",QMessageBox.Yes|QMessageBox.No,QMessageBox.No)==QMessageBox.Yes
        authority=QMessageBox.question(self,"Stap 5 van 5 â€” Bevoegdheid","Is vastgesteld dat deze persoon dit specifieke wachtwoord mag ontvangen?",QMessageBox.Yes|QMessageBox.No,QMessageBox.No)==QMessageBox.Yes
        try:
            ...
            if QMessageBox.question(self,"Laatste bevestiging",f"Toon het wachtwoord van â€˜{entry['label']}â€™ voor {requester}?\n\nDeze inzage wordt lokaal gelogd.",QMessageBox.Yes|QMessageBox.No,QMessageBox.No)!=QMessageBox.Yes:return
            secret=self.vault.reveal(entry_id,reason,verification_id); QApplication.clipboard().setText(secret); alphabet="  ".join(f"{ch} â€” {self._spell(ch)}" for ch in secret); QMessageBox.information(self,"Wachtwoord",f"{secret}\n\nSpelalfabet:\n{alphabet}\n\nHet wachtwoord staat 30 seconden op het klembord.")
