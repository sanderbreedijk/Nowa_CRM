...
class Customer360Page(QWidget):
    def __init__(self, customers: CustomerService, service: Customer360Service, open_vault, open_proposal, parent=None):
        ...
        root=QVBoxLayout(self); title=QLabel("360° klantdossier"); title.setObjectName("Title"); root.addWidget(title)
        sub=QLabel("Alle commerciële, operationele en service-informatie van één klant in één scherm."); sub.setObjectName("Subtitle"); root.addWidget(sub)
        ...
        for i,name in enumerate(("Contacten","Offertes","Kluisitems","Gebruikers","Licenties","Hardware","Open acties","Gesprekken","E-mails","Locaties","Software","Documenten","Open tickets")):
    ...
    def reload(self,*_):
        ...
        values=(len(data["contacts"]),len(data["proposals"]),len(data["vault"]),len(data["users"]),
                ...
                len([x for x in data["tickets"] if x["status"] not in ("Opgelost","Gesloten")]))
